...
const AddToCartButton = ({ product }) => {
  ...
  return (
    <>
      {
        quantity === 0 ? (
          <button onClick={increment} className="w-32 absolute bottom-[-2.6rem] left-1/2 transform -translate-x-1/2 -translate-y-1/2 whitespace-nowrap bg-white rounded-4xl border-1 border-custom-maroon text-[0.8rem] text-custom-rose-900 font-bold p-3 cursor-pointer">
        ...
        (
          <div className="flex justify-between items-center w-32 absolute bottom-[-2.6rem] left-1/2 transform -translate-x-1/2 -translate-y-1/2 whitespace-nowrap bg-custom-maroon rounded-4xl border-1 border-custom-maroon text-[0.8rem] text-white font-bold p-3">
            <button onClick={decrement} className="w-3 h-3 flex justify-center cursor-pointer">
              <img src={ minusIcon } alt="minus" />
            </button>
            { quantity }
            <button onClick={increment}  className="w-3 h-3  flex justify-center cursor-pointer">
